---
title: "Multiple Linear Regression {{< fa diagram-project >}}"
...
format: 
  live-revealjs: 
    output-file: 05-mlr-slides.html
...
theme_set(
  theme_classic(base_size = 19) +
    theme(
      ...
      axis.title = element_text(size = 25),   # Both axis titles (X and Y)
      axis.text = element_text(size = 14),    # Both axis tick labels
...
```{r}
#| out-width: 100%
knitr::include_graphics("./images/05-mlr/yt.svg")
```
:::
...
```{r}
#| out-width: 100%
knitr::include_graphics("./images/05-mlr/fb.jpeg")
```
:::
...
```{r}
#| out-width: 100%
knitr::include_graphics("./images/05-mlr/ig.jpeg")
```
:::
...
- [**SLR**]{.green}: Each slope $\beta_k$ is a **marginal** comparison. It does not adjust for the other advertising channels.

- [**MLR**]{.green}: $\beta_k$ is a **conditional** comparison. $\beta_k$ is the *expected difference* in sales, *comparing* two companies who
differ by one unit in spending in channel $k$ while **being equal in all the other channels**.
...
```{r}
#| fig-cap: "Source: https://didyousaydata.xyz/post/2018-09-05-linear-regression-modeling-and-assumptions/"
#| out-width: 46%
knitr::include_graphics("./images/05-mlr/mlr_depress.jpeg")
```
...
- MLR is not just “SLR with more variables.” 
- When a coefficient changes after other predictors enter the model, the question has changed.
...
```{r}
#| fig-cap: "Source: https://www.tldrpharmacy.com/content/how-to-be-awesome-at-biostatistics-and-literature-evaluation-part-iii"
#| out-width: 82%
knitr::include_graphics("./images/05-mlr/mlr_meme.jpeg")
```
...
quantifying the association between $X_j$ and $Y$.
...
<!-- \hline -->
<!-- \text{Observation},\, i & \text{Response},\, y & \multicolumn{4}{c}{\text{Regressors}} \\ -->
<!-- \cline{3-6} -->
<!--  &  & x_1 & x_2 & \cdots & x_k \\ -->
<!-- \hline -->
...
rsm::contour.lm(my_lm, x2 ~ x1, image = TRUE, img.col = cm.colors(50),
                cex.lab = 1.4, lwd = 2, cex.axis = 1.5, las = 1)
```
...
rsm::contour.lm(my_lm_interact, x2 ~ x1, image = TRUE, 
                ...
                cex.lab = 1.4, lwd = 2, cex.axis = 1.5)
```
...
The function $SS_{res}(\cdot)$ must be minimized with respect to the coefficients, i.e.,
...
## [**Delivery Time Data**](./data/data-ex-3-1.csv)
...
p <- plot_ly(
  ...
  layout(
    ...
    scene = list(
      xaxis = list(title = "Cases", range = c(2, 30)),
      yaxis = list(title = "Distance", range = c(36, 1640)),
      zaxis = list(title = "Time", range = c(8, 80))
    )
  )
...
```{r}
fit_case <- lm(time ~ cases, data = delivery_data)
```

$$\widehat{\text{time}} = `r coef(fit_case)[1]` + `r coef(fit_case)[2]` \text{ cases}$$
...
```{r}
fit_dis <- lm(time ~ distance, data = delivery_data)
```

$$\widehat{\text{time}} = `r coef(fit_dis)[1]` + `r coef(fit_dis)[2]` \text{ distance}$$
...
```{r}
del_long <- delivery_data |>
  pivot_longer(c(cases, distance), names_to = "predictor", values_to = "x") |>
  mutate(predictor = recode(predictor, cases = "cases", distance = "distance"))

p <- ggplot(del_long, aes(x, time)) +
  geom_point(size = 3, alpha = 0.75) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1.1) +
  facet_wrap(~ predictor, scales = "free_x", nrow = 1) +
  labs(
    title = "Delivery time is related to both cases and distance",
    x = NULL,
    y = "Delivery time"
  )
print(p)
...
#| echo: true
#| code-line-numbers: false
fit_delivery <- lm(time ~ cases + distance, data = delivery_data)
coef(fit_delivery)
```

$$\widehat{\text{time}} = 2.34 + 1.62 \text{ cases} + 0.014 \text{ distance}$$
...
```{r}
# Data for delivery effect plots
case_grid <- seq(min(delivery_data$cases), max(delivery_data$cases), length.out = 80)
dist_values <- quantile(delivery_data$distance, probs = c(0.25, 0.50, 0.75))
effect_cases <- crossing(cases = case_grid, distance = as.numeric(dist_values))
effect_cases$fit <- predict(fit_delivery, newdata = effect_cases)
effect_cases <- effect_cases |>
  mutate(
    distance_level = factor(
      paste0("distance = ", round(distance), " ft"),
      levels = paste0("distance = ", round(as.numeric(dist_values)), " ft")
    )
  )
dist_grid <- seq(min(delivery_data$distance), max(delivery_data$distance), length.out = 80)
case_values <- quantile(delivery_data$cases, probs = c(0.25, 0.50, 0.75))
effect_distance <- crossing(distance = dist_grid, cases = as.numeric(case_values))
effect_distance$fit <- predict(fit_delivery, newdata = effect_distance)
effect_distance <- effect_distance |>
  mutate(
    cases_level = factor(
      paste0("cases = ", round(cases)),
      levels = paste0("cases = ", round(as.numeric(case_values)))
    )
  )
```
...
p <- ggplot() +
  geom_point(data = delivery_data, aes(cases, time), size = 4, alpha = 0.65) +
  geom_line(data = effect_cases, aes(cases, fit, group = distance_level, linetype = distance_level, color = distance_level), linewidth = 1.5) +
    annotate(
    "text",
    x = -Inf, y = Inf,
    label = paste(
  ...
  labs(
    title = "Predicted delivery time vs. cases",
    subtitle = "Each line fixes distance at a representative value",
    x = "Number of cases",
...
#| out-width: 100%
p <- ggplot() +
  geom_point(data = delivery_data, aes(distance, time), size = 4, alpha = 0.65) +
  geom_line(data = effect_distance, aes(distance, fit, group = cases_level, linetype = cases_level, color = cases_level), linewidth = 1.5) +
  labs(
    title = "Predicted delivery time vs. distance",
    subtitle = "Each line fixes cases at a representative value.",
    x = "Distance (ft)",
    y = "Predicted delivery time",
    linetype = NULL,
    color = NULL
...
# regression plane
plot3d$plane3d(fit_delivery, lty.box = "solid", draw_lines = TRUE,
               draw_polygon = TRUE, lwd = 0.1,
               polygon_args = list(border = "green", col = rgb(0, 1, 0, 0.2)))

# overlay positive residuals
res_pos <- resid(fit_delivery) > 0
plot3d$points3d(x[res_pos], y[res_pos], z[res_pos], pch = 16, col = "blue", cex = 0.8)

# compute locations of segments
orig     <- plot3d$xyz.convert(x, y, z)
plane    <- plot3d$xyz.convert(x, y, fitted(fit_delivery))
i.negpos <- 1 + (resid(fit_delivery) > 0) # which residuals are above the plane?
...
z_hat <- fitted(fit_delivery)
res <- resid(fit_delivery)


# --------------------------------------------------
# 2. Create grid for fitted regression plane
# --------------------------------------------------

x_seq <- seq(min(x), max(x), length.out = 30)
...
plane_grid$fit <- predict(
  fit_delivery,
  newdata = plane_grid
)

# Plotly surface needs a matrix:
# rows = distance
# columns = cases
...
# --------------------------------------------------
# 3. Function for residual line segments
# --------------------------------------------------

make_segments <- function(ind) {

  list(
    x = as.vector(rbind(x[ind], x[ind], NA)),
    y = as.vector(rbind(y[ind], y[ind], NA)),
    z = as.vector(rbind(z[ind], z_hat[ind], NA))
  )
...
seg_pos <- make_segments(pos)
seg_neg <- make_segments(neg)
...
p <- plot_ly(
  ...
  add_markers(
    ...
    text = ~paste0(
      "Cases: ", x,
      "<br>Distance: ", y,
      "<br>Observed time: ", round(z, 1),
      "<br>Fitted time: ", round(z_hat, 1),
      "<br>Residual: ", round(res, 1)
    ),
    hoverinfo = "text",
    name = "Observed"
  ) %>%
...
## [R Lab]{.pink} Estimation of $\sigma^2$ {visibility="hidden"}

```{r}
#| echo: true

## method 1
summ_delivery <- summary(fit_delivery)  ## check names(summ_delivery)
...
## method 2
n <- length(fit_delivery$residuals)
(SS_res <- sum(fit_delivery$residuals * fit_delivery$residuals))
SS_res / (n - 3)
```
...
## method 3
(SS_res1 <- sum((delivery_data$time - fit_delivery$fitted.values) ^ 2))
SS_res1 / (n - 3)
```
...
## Properties of LSEs {visibility="hidden"}
...
<!-- # #| out-width: 100% -->
<!-- # par(mgp = c(2.8, 0.9, 0), mar = c(4, 4, 2, 0)) -->
<!-- # car::confidenceEllipse(fit_delivery, levels = 0.95, which.coef = c("cases", "distance"),  -->
...
par(mar = c(3, 3, 1, 0))
plot(effects::predictorEffects(predictor = "cases", mod = fit_delivery))
```
...
par(mar = c(3, 3, 1, 0))
plot(effects::predictorEffects(predictor = "distance", mod = fit_delivery))
```
...
- plot(effects::predictorEffects(mod = fit_delivery, confint = list(type = "Scheffe")))
...
```{r}
#| out-width: 100%
knitr::include_graphics("./images/05-mlr/partition.png")
```
...
#| echo: true
#| code-line-numbers: false
anova(fit_delivery) ## This is for sequential F-test
```
...
<!-- # ```{r} -->
<!-- # #| out-width: 100% -->
<!-- # knitr::include_graphics("./images/05-mlr/anova_ex.png") -->
<!-- # ``` -->
...
When OLS adds a predictor, it can always set the new coefficient to zero. Therefore, RSS cannot increase and ordinary R2 cannot decrease.
...
<!-- :::  notes -->
<!-- qt(0.05/2, df = fit_delivery$df.residual, lower.tail = FALSE) ## t critical value df = n - 3 = 22 -->
<!-- ```{r} -->
<!-- XtX_inv_diag <- diag(solve(t(X) %*% X)) ## diagonal elements of (XtX)^-1 -->
<!-- se_b <- sqrt(summ_delivery$sigma ^ 2 * XtX_inv_diag) ## standard error -->
<!-- (t_test <- (fit_delivery$coefficients[3] - 0) / se_b[3]) ## t test statistic -->
...
- An estimate of \$10000 with a standard error of \$10000 would
not be statistically significant, but it has the possibility of being important in practice.
...
A result can be statistically significant—not easily explainable by chance alone—but without being
large enough to be important in practice. For example, if a treatment is estimated to increase earnings
by $10 per year with a standard error of $2, this would be statistically but not practically significant
(in the U.S. context). Conversely, an estimate of $10 000 with a standard error of $10 000 would
not be statistically significant, but it has the possibility of being important in practice (and is also
...
par(mar = c(4, 5, 0, 0))
plot(X[, -1], pch = 16, las = 1, xlab = "cases", ylab = "distance", cex = 2,
     cex.lab = 1.5)
points(xa[1], xa[2], col = 2, pch = 15, cex = 3)
points(xb[1], xb[2], col = 4, pch = 17, cex = 3)
legend("topleft", c("interpolate", "extrapolate"), col = c(2, 4), 
       pch = c(15, 17), bty = "n", cex = 2)
...
::: notes
Causal interpretation comes from
research design+causal assumptions+appropriate analysis,
:::


<!-- - Not Within Units -->

<!-- - The data do not directly tell us what would happen if $X_k$ were changed within the same individual. -->


<!-- - [Avoid Causal] *“Increasing $X_k$ by 1 causes $Y$ to increase by $\beta_k$.”Z* -->

<!-- - Use “difference” or “comparison” rather than “change” or “causes” unless a causal interpretation is justified. -->



<!-- ## Counterfactual and Predictive Interpretations -->

<!-- - The **predictive** interpretation: how the outcome differs, on average, when comparing **two** groups of items that differ by 1 in the relevant predictor while being identical in all the other predictors. -->

<!-- - The **counterfactual** interpretation: expressed in terms of changes **within** individuals, rather than comparisons between individuals. -->
<!--   + $\text{kid_score} = 26 + 0.6 \text{ mom_IQ} + \text{error}$  -->
<!--   + changing maternal IQ from 100 to 101 would lead to an expected increase of 0.6 in child’s test score. -->
<!--   + This sort of interpretation arises in causal inference. -->

<!-- - From the data alone,  -->

<!-- - Thus, the most careful interpretation of regression coefficients is in terms of comparisons, for example, “When comparing two children whose mothers have the same level of education, the child whose mother is x IQ points higher is predicted to have a test score that is 6x higher, on average.” Or, “Comparing two items i and j that differ by an amount x on predictor k but are identical on all other predictors, the predicted difference yi - yj is $\beta_k x$, on average.” -->


<!-- ::: notes -->

<!-- Under the linear model, the coefficient is the expected difference in y -->
