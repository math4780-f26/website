...
draw_base_panel <- function() {
  ...
  # blue density curve
  lines(x, y, col = "#1f77b4", lwd = 2)
  
  # blue horizontal baseline
  segments(-3.35, 0, 3.35, 0, col = "#1f77b4", lwd = 1.2)
  
  # black separator line near bottom
  segments(-3.35, -0.13, 3.35, -0.13, col = "black", lwd = 1)
}

draw_hypothesis <- function(title_text, op_text) {
  
  # Title
  text(-3.15, 0.96, labels = title_text, adj = c(0, 1),
       cex = 2.55, font = 2)
  
  # Hypothesis pieces
  text(-3.15, 0.78, labels = expression(paste(H[a], ":", " ")), adj = c(0, 0.5), cex = 2.5)
  text(-2.80, 0.78, labels = expression(mu),       adj = c(0, 0.5), cex = 2.5)
  text(-2.6, 0.78, labels = op_text,              adj = c(0, 0.5), cex = 2.5)
  text(-2.4, 0.78, labels = expression(mu[0]),    adj = c(0, 0.5), cex = 2.5)
  
  # Red circle around operator
  # symbols(-2.59, 0.78, circles = 0.08, inches = FALSE,
  #         add = TRUE, fg = "#ef3b0c", lwd = 2)
}

draw_vline <- function(x0) {
  segments(x0, -0.02, x0, 1.02,
           col = "#1f77b4", lwd = 2, lty = 2)
}

draw_bracket <- function(x1, x2, y = -0.085, col = "#ff7f0e") {
  segments(x1, y, x2, y, col = col, lwd = 5)
  segments(x1, y - 0.03, x1, y + 0.02, col = col, lwd = 2)
  segments(x2, y - 0.03, x2, y + 0.02, col = col, lwd = 2)
}

# ---------- Panel 1: Right-tail ----------
draw_base_panel()
draw_hypothesis("Right-tailed test", ">")
draw_vline(cval)

text(-0.68, 0.40,
     labels = expression(paste("Reject ", H[0], " if test statistic ", "\u2265", " c")),
     adj = c(0, 0.5), cex = 2.5)

draw_bracket(cval, 5)
text(1.55, -0.045,
     labels = "critical value  c",
     cex = 2.5)

# ---------- Panel 2: Left-tail ----------
draw_base_panel()
draw_hypothesis("Left-tailed test", "<")
draw_vline(-cval)
...
# ---------- Panel 3: Two-tail ----------
draw_base_panel()
draw_hypothesis("Two-tailed test", "\u2260")
draw_vline(-1.96)
draw_vline(1.96)

text(-1.83, 0.52,
     labels = expression(paste("Reject ", H[0], " if test statistic ",
                               "\u2264", " -c", " or ", "\u2265", " c")),
     adj = c(0, 0.5), cex = 2.5)
...
# Bottom x-axis label
mtext("test statistic (z or t)", side = 1, line = 0.2, cex = 1.5)
```
...
text(0, -0.01, expression(mu[0]), cex = 1.5)
...
- <span style="color:blue">This is a left-tailed test, so the $p$-value is $P(T < t_{test} \mid \mu = 150)=P(T < -2.55 \mid \mu = 150) =$ `r round(pt(-2.55,24), 2)` </span>
